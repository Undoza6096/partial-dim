...
function setNGm5Tooltip() {
	...
	//ACHIEVEMENT ROW 1.5
	TimePx.setAttribute('ach-tooltip', "Get at least " + shortenInt(1e20) + " Paradox Power.")
}
...
function setR2Tooltip() {
	...
	apocAchieve.setAttribute('ach-tooltip', "Get over " + shortenInt(1e80) + " antimatter.");
	...
	noPointAchieve.setAttribute('ach-tooltip', "Buy a single First Dimension when you have over " + shortenInt(1e150) + " of them. Reward: First Dimensions are 10% stronger" + (player.tickspeedBoosts == undefined ? "." : " and you can max buy Dimension and Tickspeed Boosts."));
...
function setR3Tooltip() {
	...
	fast.setAttribute('ach-tooltip', "Big Crunch in under 2 hours. Reward: Start with " + shortenInt(1e3) + " antimatter" + (inNGM(2) ? " and get a multiplier to galaxy points based on fastest infinity (5 hours / x, 10x softcap)." : "."));
...
function setR4Tooltip() {
	...
	//ACHIEVEMENT ROW 4
	sanic.setAttribute('ach-tooltip', "Get over " + shortenInt(1e63) + " antimatter" + (tmp.ngmX >= 4 ? " and unlock new galaxy upgrades at " + shortenInt(1e666) + " antimatter" : "") + ".")
	...
	potato.setAttribute('ach-tooltip', "Get more than " + shortenInt(1e29) + " ticks per second. Reward: Reduce the starting tick interval by 2%.");
	dimensional.setAttribute('ach-tooltip', "Reach " + shortenInt(1e12) + " of all Normal Dimensions, except for the 8th Dimension.");
...
function setR5Tooltip() {
	...
	//ACHIEVEMENT ROW 5
	forever.setAttribute('ach-tooltip', "Big Crunch in 1 minute or less. Reward: Start with "+shortenInt(1e10)+" antimatter" + (inNGM(2) && player.tickspeedBoosts == undefined ? ", and gain a multiplier to IP based on your best Infinity time." : "."))
...
function setR6Tooltip() {
	...
	oh.setAttribute('ach-tooltip', "Reach " + shortenInt(1e8) + " IP per minute." + (inNGM(2) && player.tickspeedBoosts == undefined ? " Reward: Gain a multiplier to GP based on the logarithm of your IP." : ""))
	mil.setAttribute('ach-tooltip',"Reach " + shortenInt(1e6) + " infinity power." + (inNGM(2) ? " Reward: First Dimensions are " + shortenInt(1e6) + " times stronger" : "") + (tmp.ngmX >= 4 ? " and each IC boosts g32 by 2%." : "."))
...
function setR7Tooltip() {
	...
	cant.setAttribute('ach-tooltip', "Get all Dimension multipliers over " + shortenInt(1e308) + ". Reward: All Normal Dimensions  are 10" + (inNGM(2) ? "x" : "%") + " stronger" + (tmp.ngmX >= 4 ? ", achievement multiplier multiplies GP gain and make the break infinity upgrades based on antimatter 4x more powerful." : "."))
	...
	blink.setAttribute('ach-tooltip', "Big Crunch in under 200 milliseconds. Reward: Start with " + shortenInt(2e25) + " antimatter, and all Normal Dimensions are stronger in the first 300 milliseconds of this Infinity.");
}
...
function setR8Tooltip() {
	...
	//ACHIEVEMENT ROW 8
	IPBelongs.setAttribute('ach-tooltip', "Big Crunch for " + shortenInt(1e150) + " IP. " + (!aarMod.newGameMinusVersion ? "Reward: Gain an additional 4x more IP." : ""))
	reference.setAttribute('ach-tooltip', "Get a x" + shortenDimensions(INF) + " multiplier in a single sacrifice. Reward: Sacrifices are stronger.")
	spare.setAttribute('ach-tooltip', "Reach " + shortenInt(E("1e35000")) + " antimatter. Reward: Dimensions are more powerful the more unspent antimatter you have.");
...
function setR9Tooltip() {
	...
	//ACHIEVEMENT ROW 9
	speed.setAttribute('ach-tooltip', "Big Crunch for "+shortenInt(1e200)+" IP in 2 seconds or less. Reward: All Normal Dimensions are significantly stronger in the first 5 seconds of an Infinity.")
	speed2.setAttribute('ach-tooltip', "Big Crunch for "+shortenInt(1e250)+" IP in 20 seconds or less. Reward: All Normal Dimensions are significantly stronger in the first 60 seconds of an Infinity.")
	overdrive.setAttribute('ach-tooltip', "Big Crunch with " + shortenInt(1e300) + " IP/min. Reward: Gain an additonal 4x more IP.")
	minute.setAttribute('ach-tooltip', "Reach " + shortenInt(1e260) + " infinity power. Reward: Double infinity power gain.")
...
function setR10Tooltip() {
	...
	guide.setAttribute('ach-tooltip', player.boughtDims ? "Reach " + shortenInt(E("1e1000000")) + " replicanti. Reward: Replicanti increases faster the more you have." : "Eternity with less than 10 infinities.")
...
function setR11Tooltip() {
	...
	//ACHIEVEMENT ROW 11
	over9000.setAttribute('ach-tooltip', "Get a total Sacrifice multiplier of "+shortenInt(E("1e9000"))+". Reward: Sacrifice doesn't reset your dimensions.")
	dawg.setAttribute('ach-tooltip', "Have all your past 10 Infinities be at least "+shortenMoney(INF)+" times higher IP than the previous one. Reward: Your antimatter doesn't reset when buying a Dimension Boost or Galaxy.")
...
function setR12Tooltip() {
	...
	//ACHIEVEMENT ROW 12
	infiniteIP.setAttribute('ach-tooltip', "Reach "+shortenInt(E("1e30008"))+" IP." + (player.galacticSacrifice == undefined || player.tickspeedBoosts != undefined ? "" : " Reward: Your total galaxies boost Galaxy points gain."))
	...
	feast.setAttribute('ach-tooltip', "Reach "+shortenInt(1e100)+" IP without any Infinities or First Normal Dimensions. Reward: Gain an IP multiplier based on time spent in this Infinity.")
	layer.setAttribute('ach-tooltip', "Reach "+shortenMoney(INF)+" EP." + (layerReward != "" ? " Reward: " + layerReward + "." : "")) 
	rid.setAttribute('ach-tooltip', "Reach "+shortenInt(E("1e22000"))+" IP without any time studies. Reward: Gain a multiplier to Time Dimensions based on the amount of bought Time Studies.")
	minaj.setAttribute('ach-tooltip', "Have 180 times more non-bonus Replicated Galaxies than normal galaxies. Reward: Getting a Replicanti Galaxy divides your replicanti by " + shortenMoney(INF) + " instead of resetting them to 1.")
}
...
function setR13Tooltip() {
	...
	if (tmp.ngp3_boost && !tmp.exMode) thinkingReward.push("TS141 is at least " + shortenInt(1e25) + "x")
	...
	potato3.setAttribute('ach-tooltip', "Get more than "+shortenInt(E("1e8296262"))+" ticks per second." + (potato3Reward != "" ? " Reward: " + potato3Reward + "." : ""))
	infstuff.setAttribute('ach-tooltip', "Reach "+shortenInt(E("1e140000"))+" IP without buying IDs or IP multipliers. Reward: " + (tmp.ngp3 && !tmp.bgMode ? "Outside of Eternity Challenges, you keep all your Infinity Challenges on Eternities" : "You start eternities with all Infinity Challenges unlocked and completed") + (player.meta ? ", and boost Infinitied gain by DT^0.25." : "."))
	when.setAttribute('ach-tooltip', "Reach "+shortenInt( E("1e20000"))+" replicanti. Reward: You gain replicanti 2 times faster under " + shortenMoney(INF) + " replicanti" + (tmp.ngp3_boost ? " and you can always buy max RGs." : "."))
	thinking.setAttribute('ach-tooltip', "Eternity for " + shortenInt( E("1e600")) + " EP in 1 minute or less while dilated." + (thinkingReward != "" ? " Reward: " + thinkingReward + "." : ""))
	thisis.setAttribute('ach-tooltip', "Reach " + shortenInt(E('1e20000')) + " IP without any time studies while Dilated." + (thisisReward != "" ? " Reward: " + thisisReward + "." : ""))
}
...
function setR13p5Tooltip() {
	...
	stillamil.setAttribute('ach-tooltip', "Reach " + shortenInt(1e6) + " black hole power.")
	out.setAttribute('ach-tooltip',"Get more than " + shortenInt(1e5) + " ex-dilation." + (aarMod.nguspV == undefined ? "" : " Reward: You can equally distribute ex-dilation to all repeatable dilation upgrades."))
	ridNGud.setAttribute('ach-tooltip', "Reach " + shortenInt(E("1e20000")) + " IP without any time studies or dilation upgrades while dilated.")
}
...
function setR14Tooltip() {
	...
	neverenough.setAttribute('ach-tooltip', "Reach " + shortenInt(E(tmp.ngp3 ? "1e60000" : "1e100000")) + " replicanti. Reward: " + (tmp.ngp3_boost ? "Replicated Galaxies no longer reset Replicantis." : "You can always buy max RGs."))
...
function setR15Tooltip() {
	...
	//ACHIEVEMENT ROW 15
	notenough.setAttribute('ach-tooltip', "Reach " + shorten(INF) + " meta-antimatter. Reward: Eternity points boost DT, TP, and MA, up to 2.5x.")
	noTax.setAttribute('ach-tooltip', "Have 1,100 galaxies without unlocking Time Dilation.")
	winner.setAttribute('ach-tooltip', "Go quantum in under 30 seconds. Reward: Start with " + shortenInt(1e10) + " meta-antimatter.") 
	old.setAttribute('ach-tooltip', "Reach " + shortenInt(getOldAgeRequirement()) + " antimatter. Reward: Get a multiplier to first 3 Meta Dimensions based on total antimatter.") 
	rid.setAttribute('ach-tooltip', "Reach " + shortenInt(E("1e300000")) + " IP while dilated, without having time studies, not including mastery studies.")
}
...
function setR19Tooltip() {
	...
	soLife.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(3.5e5)) + " IP in Big Rip while dilated, with no EP multiplier upgrades and time studies. Reward: Square the Ghost Particle gain, with a hardcap at " + shortenInt(1e10) + "x, and the hardcap is further lowered if you have more than " + shortenInt(1e60) + " Ghost Particles.")
}
...
function setR20Tooltip() {
	...
	//ACHIEVEMENT ROW 20
	finite.setAttribute('ach-tooltip', "Get " + shortenInt(1e33) + " Space Shards without Breaking Eternity within this Ghostify. Reward: Outside of Big Rips, Tree Upgrades are 10% stronger. In Big Rips, 8th Time Dimensions gain an small exponent boost based on your current Ghostify time.")
	really.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(5000)) + " matter in Big Rip.")
	willenough.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(1/0)) + " replicanti." + (willenoughReward != "" ? " Reward: " + willenoughReward + "." : ""))
	pls.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(9.4e5)) + " IP in Big Rip while dilated, with no EP multiplier upgrades, time studies, and Break Eternity within this Ghostify. Reward: Each time you become a ghost, you gain " + shortenDimensions(2e3) + " galaxies worth of generated neutrinos, multiplied by your best-ever galaxy amount across all Big Rips.")
}
...
function setBMTooltip() {
	...
	//BRAVE MILESTONES
	bm1.setAttribute('ach-tooltip', "Reward: Start Ghostifies with all Speedrun Milestones and all "+shorten(INF)+" QK assignation features unlocked, all Paired Challenges completed, all Big Rip upgrades bought, and you get quarks based on your best MA this quantum.")
	...
	bm14.setAttribute('ach-tooltip', "Reward: Start Ghostifies with " + shortenInt(1e25) + " Quark Spins and Branches are faster based on spins (at least 10x).")
}
...
function setR21Tooltip() {
	...
	//ACHIEVEMENT ROW 21
	uc.setAttribute('ach-tooltip', "Become a ghost with at least "+shortenInt(pow10(2.2e5))+" EP without starting Eternity Challenge 10 while Big Ripped. Reward: Meta-Dimension Boosts no longer reset Meta Dimensions.")
	...
	arent.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(1.8e6)) + " IP while dilated and Big Ripped and without having studies, EP mult upgrades, Tree Upgrades, and Break Eternity within this Ghostify. Reward: ???")
}
...
function setR22Tooltip() {
	...
	oc.setAttribute('ach-tooltip', "Become a ghost with at least " + shortenInt(pow10(3.75e5)) + " EP while Big Ripped with the Anti-Dilation modifier.")
	btco.setAttribute('ach-tooltip', "Complete Paired Challenge 1 after getting " + shortenInt(pow10(1.65e9)) + " antimatter in Quantum Challenges 6 and 8. Reward: Ghostifies only makes you lose 25% of your radiocative decays.")
	tdc.setAttribute('ach-tooltip', "Complete Eternity Challenge 11 with " + shortenInt(pow10(15500)) + " IP in a Paired Challenge with the Quantum Challenges 6 and 8 combination and the Anti-Dilation modifier. Reward: Remove the quadratic cost scaling and the level softcap of fifth Tree of Decay upgrade and make it based on best meta-antimatter over Ghostifies, instead of over quantums.")
...
function setR23Tooltip() {
	...
	//ACHIEVEMENT ROW 23
	aretheseanother.setAttribute('ach-tooltip', "Reach " + shortenInt(pow10(4e4)) + " Quarks. Reward: Gain 500x more Quarks and Ghost Particles and always gain 1% of your Eternity Points upon Eternity per second.")
	ghostliest.setAttribute('ach-tooltip', "Get " + shorten(Math.pow(INF, 1/4)) + " Ghostifies. Reward: Ghostifies boost the gain of Ghost Particles at a reduced rate.")
	metae18.setAttribute('ach-tooltip', "Get " + shortenInt(pow10(1e18)) + " antimatter. Reward: Distant Antimatter Galaxies scaling is 10% weaker and start with W Bosons and Z Bosons based on Higgs Bosons.")
}
...
function setR24Tooltip() {
	...
	x.setAttribute('ach-tooltip', 'Get 242.4 PC ranking. Reward: Start with ' + shortenInt(1e25) + ' Space Shards and Eternal Matter.')
	how.setAttribute('ach-tooltip', 'Reach ' + shortenInt(pow10(Math.sqrt(2) * 1e12)) + ' antimatter in Big Rips.')
}
